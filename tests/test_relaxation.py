...
def test_no_relaxation_needed_when_results_exist() -> None:
    ...
    result, final_filters, steps = relax_and_search(all_listings(), filters)
...
def test_relaxation_kicks_in_on_zero_results_and_stops_at_first_success() -> None:
    ...
    baseline = relax_and_search(all_listings(), SearchFilters(body_type=BodyType.WAGON))
...
def test_relaxation_order_is_radius_then_mileage_then_price_then_year() -> None:
    ...
    _result, _final_filters, steps = relax_and_search(all_listings(), filters)
